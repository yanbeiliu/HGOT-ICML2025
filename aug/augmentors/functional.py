...
from aug.utils import normalize
...
def multiinstance_mixup(x1: torch.Tensor, x2: torch.Tensor,
                        alpha: float, shuffle=False) -> typing.Tuple[torch.Tensor, torch.Tensor]: # 原先为(), 改成typing.Tuple[]
    """
...
def coalesce_edge_index(edge_index: torch.Tensor, edge_weights: Optional[torch.Tensor] = None) -> typing.Tuple[torch.Tensor, torch.FloatTensor]:
    num_edges = edge_index.size()[1]
    num_nodes = edge_index.max().item() + 1
...
def drop_node(edge_index: torch.Tensor, edge_weight: Optional[torch.Tensor] = None, keep_prob: float = 0.5) -> typing.Tuple[torch.Tensor, torch.Tensor, Optional[torch.Tensor]]:
    num_nodes = edge_index.max().item() + 1
    ...
    return subset, edge_index, edge_weight
...
def random_walk_subgraph(edge_index: torch.LongTensor, edge_weight: Optional[torch.FloatTensor] = None, batch_size: int = 1000, length: int = 10):
    ...
    return edge_index, edge_weight, node_idx
