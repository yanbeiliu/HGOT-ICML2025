...
class Graph(NamedTuple):
    ...
    subset: Optional[torch.BoolTensor]

    def unfold(self) -> Tuple[torch.FloatTensor, torch.LongTensor, Optional[torch.FloatTensor], Optional[torch.BoolTensor]]:
        return self.x, self.edge_index, self.edge_weights, self.subset
...
class Augmentor(ABC):
    ...
    def __call__(
            ...
            edge_index: torch.LongTensor, edge_weight: Optional[torch.FloatTensor] = None, subset: Optional[torch.BoolTensor] = None
    ) -> Tuple[torch.Tensor, torch.Tensor, Optional[torch.Tensor], Optional[torch.Tensor]]:
        return self.augment(Graph(x, edge_index, edge_weight, subset)).unfold()
